...
\begin{itemize}
    ...
    \item A role-based authentication system using Firebase Auth distinguishing between Student and Admin users.
...
Sandhu and Samarati \cite{sandhu1994access} established foundational principles of Role-Based Access Control (RBAC), which remain the dominant paradigm for academic information systems. TeamSync implements a lightweight RBAC model with two roles --- Student and Admin --- enforced using Firebase Authentication and Firestore security rules.

% ---------------------------------------------------------------------------
\section{System Architecture}

\subsection{Overview}
TeamSync follows a modern serverless architecture with a clearly separated frontend SPA (Single Page Application) and Firebase backend services. Figure~\ref{fig:architecture} presents the system architecture.
...
\begin{tikzpicture}[node distance=0.6cm and 0.5cm]
  ...
  % API layer
  \node[box, fill=orange!20, minimum width=5.2em, below=0.9cm of router] (express) {Firebase SDK\\{\tiny Client Libs}};
  \node[box, fill=orange!20, minimum width=5.2em, left=0.4cm of express] (authmw) {Firebase Auth\\{\tiny Authentication}};
  \node[box, fill=orange!20, minimum width=5.2em, right=0.4cm of express] (adminmw) {Role Mgmt\\{\tiny Firestore}};

  \begin{pgfonlayer}{background}
    \node[fit=(authmw)(express)(adminmw), fill=orange!5, draw=orange!50, rounded corners,
          label={[font=\scriptsize]above:Backend Services Layer}, inner sep=4pt] (be) {};
  \end{pgfonlayer}

  % DB layer
  \node[dbbox, below=0.9cm of express] (mongo) {Firestore DB\\{\tiny Real-time NoSQL}};

  % Arrows
  \draw[arrow] (browser) -- (router);
  \draw[arrow] ([xshift=0.05cm]fe.south) -- ([xshift=0.05cm]be.north) node[midway, right, font=\tiny] {Secure Calls};
  \draw[arrow] ([xshift=-0.05cm]be.north) -- ([xshift=-0.05cm]fe.south);
  \draw[arrow] (express) -- (mongo) node[midway, right, font=\tiny] {Queries/Listeners};
...
The frontend is built with \textbf{React 18} \cite{react2022} using functional components and hooks. \textbf{Vite} is used as the build tool and development server for its significantly faster hot module replacement (HMR) compared to Create React App. The UI leverages \textbf{Tailwind CSS} for utility-first styling and \textbf{Framer Motion} for page transitions. Client-side routing is handled by \textbf{React Router v6}, supporting protected routes that verify Firebase authentication state before rendering.

\subsection{Backend Architecture}
The backend leverages a \textbf{serverless architecture using Firebase} \cite{firebase2023}. Data persistence is provided by \textbf{Firestore}, a flexible, scalable NoSQL cloud database. Authentication state is managed via \textbf{Firebase Authentication}, enabling both email/password sign-in and Google OAuth restricted specifically to the \texttt{mitaoe.ac.in} domain to ensure platform exclusivity.

\subsection{Authentication \& Authorization}
Figure~\ref{fig:authflow} illustrates the complete authentication flow. Upon successful login (via email/password or Google OAuth), Firebase Auth manages session state and provides a secure user object. The application subsequently fetches the user's role from their associated Firestore document.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[node distance=0.45cm]
  \node[flowbox, fill=gray!20] (start) {User visits Login Page};
  \node[flowbox, below=of start] (creds) {Firebase Auth Sign-In};
  \node[flowbox, below=of creds] (post) {Verify @mitaoe.ac.in domain};
  \node[decision, below=of post] (valid) {Valid\\Domain?};
  \node[flowbox, right=0.6cm of valid, fill=red!15] (err) {Return 401\\Error};
  \node[flowbox, below=of valid] (jwt) {Fetch User Doc from Firestore};
  \node[flowbox, below=of jwt] (store) {Store State in localStorage};
...
\end{tikzpicture}
\caption{Firebase authentication and role-routing flow.}
\label{fig:authflow}
\end{figure}
...
The core data entity is the \textbf{User} document, stored in a Firestore collection with the following primary fields:
...
\hline
password & String & Managed by Firebase Auth \\
\hline
...
\subsection{Data Management Pattern}
Instead of traditional REST APIs, TeamSync utilizes Firebase SDK client-side data fetching. Data is organized into structured Firestore collections such as `users`, `teams`, and `messages`. This architecture drastically reduces backend operational complexity while ensuring real-time data sync across all connected clients.
...
Upon login, the application reads the \texttt{role} field from the user's Firestore document and routes accordingly:
\begin{itemize}
    \item \textbf{Student Dashboard}: Displays event participation metrics, team status, active chats, and matched teammates. Real-time Firestore listeners update the UI instantly when a match or message occurs.
    \item \textbf{Admin Dashboard}: Provides user management with Firestore role update capabilities, platform-wide analytics visualized using Firebase Analytics data, and event management tools.
\end{itemize}
...
\hline
Backend & Firebase Cloud Platform & Serverless backend \\
\hline
Database & Firestore & Real-time NoSQL DB \\
\hline
Auth & Firebase Authentication & Auth, strict domain mgmt \\
\hline
...
\begin{itemize}
    \item Student registration and login with Firebase Auth.
...
\end{itemize}
All flows functioned correctly with appropriate error handling for invalid institutional credentials and unauthorized access attempts handled by Firestore Security Rules.
...
This paper presented \textbf{MITAOE TeamSync}, a web-based platform addressing the persistent problem of ad hoc team formation for academic hackathons. By combining a swipe-driven matching interface, domain-specific skill profiling, strict Firebase-based role authentication, and an integrated chat module, TeamSync provides MITAOE students with a structured and engaging teammate discovery experience. Built almost entirely on serverless Firebase technologies and open-source frontend frameworks at a total cost of Rs.~58.00, the platform demonstrates that high-quality, feature-rich academic software can be delivered with minimal financial resources. The system serves as both a practical tool for students and a proof-of-concept for AI-augmented team formation in future iterations.
...
Google LLC, ``Firebase -- An app development platform that helps you build and grow apps and games,'' 2023. [Online]. Available: \url{https://firebase.google.com}
